...
namespace LctTrainingDemo.Authorization
{
    public class BasicAuthenticationAndAuthorization :AuthorizationFilterAttribute
    {
        ...
        public override void OnAuthorization(HttpActionContext actionContext)
        {
            ...
            else
            {
                ...
                string[] userCredentials = decodeAuthenticationToken.Split(':');
                string userName = userCredentials[0];
                string password = userCredentials[1];
                if(uName.Equals(userName, StringComparison.OrdinalIgnoreCase) && password.Equals(password))
                {
